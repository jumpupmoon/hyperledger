...
function replaceCAKey() {
    ...
    sed -i "s/CA_PRIVATE_KEY3/${PRIV_KEY3}/g" docker-compose.yml
}

# crypto-config / config 폴더 존재여부 확인
...
docker-compose -f docker-compose.yml up -d ca1.example.com ca2.example.com ca3.example.com orderer.example.com peer0.org1.example.com peer0.org2.example.com peer0.org3.example.com couchdb1 couchdb2 couchdb3 cli #수정
docker ps -a
...
# Create the channel
docker exec cli peer channel create -o orderer.example.com:7050 -c mychannel -f /etc/hyperledger/configtx/channel.tx # 수정
sleep 3 #추가 
# Join peer0.org1.example.com to the channel.
docker exec -e "CORE_PEER_LOCALMSPID=Org1MSP" -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org1.example.com/msp" peer0.org1.example.com peer channel join -b /etc/hyperledger/configtx/mychannel.block #수정
# 추가
sleep 3 
